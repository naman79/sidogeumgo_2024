     ...
     GYEJWA AS (
         ...
         GROUP BY   C.LAF_CD
             ...
             GEORAE_JAN_SEQ AS (
                 ...
                 FROM  ACL_SIGUMGO_SLV A
                           INNER JOIN BATCH_PARAM D ON 1=1
                           INNER JOIN GEORAE_JAN_SEQ B ON A.SIGUMGO_ORG_C = B.SIGUMGO_ORG_C AND A.TRXDT = B.TRXDT AND A.FIL_100_CTNT5 = B.GONGGEUM_GYEJWA AND A.LINKAC_TRX_NO = B.MAX_TRX_NO
                 WHERE 1=1
                   AND A.SIGUMGO_ORG_C = D.GEUMGO_CODE
                   AND A.FIL_100_CTNT5 IN (
                     SELECT GONGGEUM_GYEJWA FROM GYEJWA
