-- 강릉시 금고운용현황(예치기간별)
-- 예금운용현황(예치기간별)은 공금잔액은 표시하지 않기에 금고운용현황에서 빼줌
...
FROM (
    ...
    GROUP BY T2.GONGGEUM_GYEJWA, T2.UNYONG_GYEJWA, T3.SIGUMGO_AC_NM
    ...
    GROUP BY  T100.GONGGEUM_GYEJWA , T100.UNYONG_GYEJWA, T300.SIGUMGO_AC_NM
...
WHERE 1 = 1
GROUP BY  T1000.GONGGEUM_GYEJWA_NO, T1000.UNYONG_GYEJWA, T1000.GONGGEUM_GYEJWA_NM
ORDER BY  T1000.GONGGEUM_GYEJWA_NO, T1000.UNYONG_GYEJWA, T1000.GONGGEUM_GYEJWA_NM
    ...
    (
        ...
        SELECT
            TO_NUMBER(T1.CMM_DTL_C) AS GEUMGO_CODE
             ,'00000000000000' AS GONGGEUM_GYEJWA
             ,'00000000000000'AS UNYONG_GYEJWA
...
WHERE UNYONG_GYEJWA = '200751673542'
...
SELECT
    A.UNYONG_GYEJWA,
    SUM(A.JANAEK) AS JAN,
    COUNT(A.UNYONG_GYEJWA) AS CNT,
    SUM(A.JANAEK) / COUNT(A.UNYONG_GYEJWA) AS P_JAN,
    TRUNC(SUM(A.JANAEK) / 366) AS YP_JAN
FROM
(
SELECT
    B.UNYONG_GYEJWA,
    ...
    B.JANAEK
FROM MAP_JOB_DATE A
LEFT OUTER JOIN RPT_UNYONG_JAN B ON B.UNYONG_GYEJWA = '200778556436' AND A.BIZ_DT = B.KIJUNIL
WHERE 1=1
AND A.DW_BAS_DDT BETWEEN '20240101' AND '20241231'
AND B.JANAEK IS NOT NULL
    ) A
...
FROM MAP_JOB_DATE T1
   ...
   , RPT_UNYONG_GYEJWA T3
WHERE 1 = 1
  AND T3.UNYONG_GYEJWA = '200778556436'
  AND T1.DW_BAS_DDT BETWEEN '20240101' AND '20241231'
  AND T2.KIJUNIL = CASE WHEN T1.DT_G IN (0) THEN T1.DW_BAS_DDT ELSE T1.BF1_BIZ_DT END
  AND T2.GEUMGO_CODE = 150
  AND SUBSTR(T2.GONGGEUM_GYEJWA, 4, 3) = LPAD('0', 3, '0')
