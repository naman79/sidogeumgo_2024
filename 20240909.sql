...
SELECT
B.REF_CTNT1 AS GONGGEUM_GYEJWA,
A.KEORAEIL,
A.SUNAPIL,
A.GUNGU_CODE,
A.HOIGYE_YEAR,
A.JOJEONG_GUBUN,
A.IPJI_GUBUN,
A.JIBANGSE_AMT * DECODE(A.IPJI_GUBUN, 2, -1, 1) AS JIBANGSE_AMT
FROM RPT_DANGSEIPJOJEONG A
LEFT JOIN RPT_CODE_INFO B ON A.JIBGYE_CODE = B.REF_D_C
WHERE B.REF_L_C = 879 
AND A.GEUMGO_CODE = 28
AND A.KEORAEIL LIKE '2024%'


SELECT
A.GONGGEUM_GYEJWA,
SUM(A.JIBANGSE_AMT) AS JIBANGSE_AMT
FROM
(
SELECT
B.REF_CTNT1 AS GONGGEUM_GYEJWA,
...
AND A.GEUMGO_CODE = 28
AND A.KEORAEIL LIKE '2024%'
ORDER BY B.REF_CTNT1, A.SUNAPIL
...
A.GJDT AS SUNAPIL,
A.SIGUMGO_TRX_G,
DECODE(A.IPJI_G,1,A.TRAMT, 2, A.TRAMT * -1,0) * DECODE(A.CRT_CAN_G,1,-1,2,-1,33,-1,1) AS JIBANGSE_AMT
FROM ACL_SIGUMGO_SLV A 
LEFT JOIN RPT_CODE_INFO B ON A.FIL_100_CTNT5 = B.REF_CTNT1
WHERE A.SIGUMGO_ORG_C = 28
AND B.REF_L_C = 879
AND A.TRXDT LIKE '2024%'
) A
GROUP BY A.GONGGEUM_GYEJWA, A.SIGUMGO_TRX_G
ORDER BY A.GONGGEUM_GYEJWA, A.SIGUMGO_TRX_G
...
(
    ...
    WHERE FIL_100_CTNT2 = B.REF_CTNT1
    AND MNG_NO = 1
) AS GONGGEUM_GYEJWA_NM,
(
    SELECT
    REF_CTNT2
    FROM RPT_CODE_INFO
...
WHERE B.REF_L_C = 879 
AND A.GEUMGO_CODE = 28
AND B.REF_CTNT1 IN ('02800075690000623', '02800075690000624', '02800075690001024', '02811075690000099', '02817075690000023', '02823775690001023')
ORDER BY B.REF_CTNT1, A.GUNGU_CODE, A.KEORAEIL, A.SUNAPIL


--------------------------------------------------------------------------------------------

SELECT
B.REF_CTNT1 AS "공금계좌번호",
(
    SELECT 
    SIGUMGO_AC_NM
    FROM ACL_SIGUMGO_MAS
...
A.KEORAEIL AS "거래일자",
A.SUNAPIL AS "수납일자",
A.HOIGYE_YEAR AS "회계년도",
A.JIBANGSE_AMT * DECODE(A.IPJI_GUBUN, 2, -1, 1) AS "지방세금액"
FROM RPT_DANGSEIPJOJEONG A
LEFT JOIN RPT_CODE_INFO B ON A.JIBGYE_CODE = B.REF_D_C
WHERE B.REF_L_C = 879 
AND A.GEUMGO_CODE = 28
...
AND A.GEUMGO_CODE = 28
AND A.KEORAEIL LIKE '2024%' 
AND B.REF_CTNT1 NOT IN ('02800075690000623', '02800075690000624', '02800075690001024', '02811075690000099', '02817075690000023', '02823775690001023')
ORDER BY B.REF_CTNT1, A.GUNGU_CODE, A.KEORAEIL, A.SUNAPIL
------------------------------------------------------------------------------
...
(
   ...
   (
                 ...
                 (
                    ...
                    THEN  IPGEUM_AMT * JO ELSE 0 END JEONYONG_IP,
                                      ...
                                      0 AS JEONYONG_JI2,
                                      0 AS JEONYONG_IP2,
                                      CASE WHEN ILJA BETWEEN '00000101' AND '20240906' AND  GEORAE_GUBUN = 11  
                    THEN  IPGEUM_AMT * JO ELSE 0 END TAX_SUNP_IP_AMT3,        
                                               CASE WHEN ILJA BETWEEN '00000101' AND '20240906' AND  GEORAE_GUBUN IN (15,16)
                    THEN  IPGEUM_AMT * JO ELSE 0 END TAX_BAEJUNG_IP_AMT3,               
                                               CASE WHEN ILJA BETWEEN  '00000101' AND '20240906' AND  GEORAE_GUBUN = 64  
        ...
        (
                            ...
                            DECODE(A.IPJI_G,1,A.TRAMT,0) IPGEUM_AMT, DECODE(A.IPJI_G,2,A.TRAMT,0) JIGEUB_AMT,
                                    ...
                                    FROM  ACL_SIGUMGO_SLV A
                                    WHERE  A.GJDT BETWEEN (CASE WHEN A.SIGUMGO_HOIKYE_YR = 9999 THEN '20240101' ELSE '00000101' END) AND '20240906' 
                                    AND A.SIGUMGO_ORG_C = '28'                                    
                                    AND A.ICH_SIGUMGO_GUN_GU_C LIKE '0' || '%'
                                    AND A.FIL_100_CTNT5 = '02800075690000124'
        )
                      UNION ALL         
                      SELECT
                                     GONGGEUM_GUNGU_CODE AS GUNGU_CODE,
        ...
        (
                      ...
                      DECODE(A.IPJI_G,1,A.TRAMT,0) IPGEUM_AMT, DECODE(A.IPJI_G,2,A.TRAMT,0) JIGEUB_AMT,
                                          ...
                                          CASE WHEN A.SIGUMGO_HOIKYE_YR = 9999 THEN TO_NUMBER(SUBSTR(A.GJDT,1,4))
                                                  ELSE TO_NUMBER(A.SIGUMGO_HOIKYE_YR) END R_HOIGYE_YEAR 
                         FROM ACL_SIGUMGO_SLV A
                         WHERE A.GJDT BETWEEN '20240906' AND '20240906'
                         AND A.SIGUMGO_ORG_C = '28'                         
                         AND A.ICH_SIGUMGO_GUN_GU_C LIKE '0' || '%'
                         AND A.FIL_100_CTNT5 = '02800075690000124'
        )
        ...
        (
                             ...
                             AND KEORAEIL IN (SELECT BIZ_DT FROM MAP_JOB_DATE WHERE DW_BAS_DDT = TO_CHAR(TO_NUMBER(SUBSTR('20240906',0,4)) -1)||'1231')
                             AND GONGGEUM_GYEJWA = '02800075690000124'
        )
 ) T,
 (
       ...
       WHERE SIGUMGO_ORG_C = '28'
             AND FIL_100_CTNT2 = '02800075690000124'
      AND MNG_NO = 1
 ) K,
 ...
 AND K.GONGGEUM_GYEJWA = '02800075690000124'
...
(
      SELECT  GUNGU_CODE, 1 DATA_ROW,
                       0 AMT1,
                       SUM(CASE WHEN JOJEONG_GUBUN = 2 AND IPJI_GUBUN  = 1 AND SUNAPIL = '20240906' THEN NVL(JIBANGSE_AMT,0) * -1
                                            WHEN JOJEONG_GUBUN = 2 AND IPJI_GUBUN  = 2 AND SUNAPIL = '20240906' THEN NVL(JIBANGSE_AMT,0)  ELSE 0 END) AMT2,
                       SUM(CASE WHEN JOJEONG_GUBUN <> 2 AND IPJI_GUBUN = 1 AND SUNAPIL = '20240906' THEN NVL(JIBANGSE_AMT,0)
                                            WHEN JOJEONG_GUBUN <> 2 AND IPJI_GUBUN = 2 AND SUNAPIL = '20240906' THEN NVL(JIBANGSE_AMT,0) * -1  ELSE 0 END) AMT3,
                       0 AMT4,
                       SUM(CASE WHEN JOJEONG_GUBUN = 2 AND IPJI_GUBUN  = 1 AND SUNAPIL = '20240906' THEN NVL(SEWOI_AMT,0) * -1
                                            WHEN JOJEONG_GUBUN = 2 AND IPJI_GUBUN  = 2 AND SUNAPIL = '20240906' THEN NVL(SEWOI_AMT,0)  ELSE 0 END) AMT5,
                       SUM(CASE WHEN JOJEONG_GUBUN <> 2 AND IPJI_GUBUN = 1 AND SUNAPIL = '20240906' THEN NVL(SEWOI_AMT,0)
                                            WHEN JOJEONG_GUBUN <> 2 AND IPJI_GUBUN = 2 AND SUNAPIL = '20240906' THEN NVL(SEWOI_AMT,0) * -1 ELSE 0 END) AMT6,
                       0 AMT7,
                       SUM(CASE WHEN JOJEONG_GUBUN = 2 AND IPJI_GUBUN  = 1 AND SUNAPIL = '20240906' THEN NVL(GYOYUKSE_AMT,0) * -1
                                            WHEN JOJEONG_GUBUN = 2 AND IPJI_GUBUN  = 2 AND SUNAPIL = '20240906' THEN NVL(GYOYUKSE_AMT,0) ELSE 0 END) AMT8,
                       SUM(CASE WHEN JOJEONG_GUBUN <> 2 AND IPJI_GUBUN = 1 AND SUNAPIL = '20240906' THEN NVL(GYOYUKSE_AMT,0)
                                            WHEN JOJEONG_GUBUN <> 2 AND IPJI_GUBUN = 2 AND SUNAPIL = '20240906' THEN NVL(GYOYUKSE_AMT,0) * -1 ELSE 0 END) AMT9,
                       0 AMT10,
                       SUM(CASE WHEN JOJEONG_GUBUN = 2 AND IPJI_GUBUN  = 1 AND SUNAPIL = '20240906' THEN NVL(NONGTEUKSE_AMT,0) * -1
                                            WHEN JOJEONG_GUBUN = 2 AND IPJI_GUBUN  = 2 AND SUNAPIL = '20240906' THEN NVL(NONGTEUKSE_AMT,0) ELSE 0 END) AMT11,
                       SUM(CASE WHEN JOJEONG_GUBUN <> 2 AND IPJI_GUBUN = 1 AND SUNAPIL = '20240906' THEN NVL(NONGTEUKSE_AMT,0)
                                            WHEN JOJEONG_GUBUN <> 2 AND IPJI_GUBUN = 2 AND SUNAPIL = '20240906' THEN NVL(NONGTEUKSE_AMT,0) * -1 ELSE 0 END) AMT12,
                       0 AMT13,
                       ...
                       SUM(CASE WHEN (SUNAPIL BETWEEN '20240906' AND '20240906') AND IPJI_GUBUN = 1 THEN NVL(JIBANGSE_AMT,0)
                                            WHEN (SUNAPIL BETWEEN SUBSTR('20240906',0,6)||'01' AND '20240906') AND IPJI_GUBUN <> 1 THEN NVL(JIBANGSE_AMT,0) * -1 ELSE 0 END) AMT25,
                       SUM(CASE WHEN (SUNAPIL BETWEEN SUBSTR('20240906',0,6)||'01' AND '20240906') AND IPJI_GUBUN =  1 THEN NVL(SEWOI_AMT,0)
                                            WHEN (SUNAPIL BETWEEN SUBSTR('20240906',0,6)||'01' AND '20240906') AND IPJI_GUBUN <> 1 THEN NVL(SEWOI_AMT,0) * -1 ELSE 0 END) AMT26,
                       SUM(CASE WHEN (SUNAPIL BETWEEN SUBSTR('20240906',0,6)||'01' AND '20240906') AND IPJI_GUBUN =  1 THEN NVL(GYOYUKSE_AMT,0)
                                            WHEN (SUNAPIL BETWEEN SUBSTR('20240906',0,6)||'01' AND '20240906') AND IPJI_GUBUN <> 1 THEN NVL(GYOYUKSE_AMT,0) * -1 ELSE 0 END) AMT27,
                       SUM(CASE WHEN (SUNAPIL BETWEEN SUBSTR('20240906',0,6)||'00' AND '20240906') AND IPJI_GUBUN =  1 THEN NVL(NONGTEUKSE_AMT,0)
                                            WHEN (SUNAPIL BETWEEN SUBSTR('20240906',0,6)||'00' AND '20240906') AND IPJI_GUBUN <> 1 THEN NVL(NONGTEUKSE_AMT,0) * -1 ELSE 0 END) AMT28
      FROM  RPT_DANGSEIPJOJEONG A
       WHERE  A.GEUMGO_CODE = '28' 
       AND A.SUNAPIL <= '20240906'
      AND A.GUNGU_CODE LIKE '0' || '%'
      AND A.JIBGYE_CODE = (SELECT REF_D_C FROM RPT_CODE_INFO WHERE REF_L_C = 879 AND REF_M_C = 28 AND REF_CTNT1 = '02800075690000124')
