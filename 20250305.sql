...
SELECT
    ...
    RPAD(NVL(A.NU_TXRV_DD_SUM_AMT, '0'), 20, ' ')|| --RCVMT_AMT -> 세입누계로 변경
    RPAD('0', 20, ' ')|| -- RFN_AMT -> 전송제외로 변경
    RPAD('0', 20, ' ')|| -- TXRV_SBJ_RVSN_AMT  -> 전송제외로 변경
    RPAD(NVL(A.TXRV_DD_SUM_AMT, '0'), 20, ' ')|| -- 세입일계
    RPAD('0', 20, ' ')|| -- FDAC_AMT  -> 전송제외로 변경
    RPAD('0', 20, ' ')|| --FDAC_RTN_AMT -> 전송제외로 변경
    RPAD('0', 20, ' ')|| --FDAC_RAMT_AMT -> 전송제외로 변경
    RPAD(NVL(A.ANE_AMT, '0'), 20, ' ')|| --EP_AMT -> 세출일계로 변경
    RPAD('0', 20, ' ')|| -- A.RTN_AMT -> 전송제외로 변경
    RPAD('0', 20, ' ')|| -- ANE_SBJ_RVSN_AMT -> 전송제외로 변경
    RPAD(NVL(A.NU_ANE_AMT, '0'), 20, ' ')|| -- ANE_AMT -> 세출누계로 변경
    RPAD('0', 20, ' ')|| --PMOD_NPYM_AMT -> 전송제외로 변경
    RPAD('0', 20, ' ')|| --FD_BDTR_AMT -> 전송제외로 변경
    RPAD(NVL(A.FD_OP_AMT, '0'), 20, ' ')|| -- FD_OP_DPI_AMT -> 운용자금일계로 변경
    RPAD('0', 20, ' ')|| --FD_OP_CNLT_AMT -> 전송제외로 변경
    RPAD(NVL(A.NU_FD_OP_AMT, '0'), 20, ' ')|| -- FD_OP_AMT -> 운용자금누계로 변경
    RPAD('0', 20, ' ')|| --STBX_RAMT_AMT -> 전송제외로 변경
    RPAD('0', 20, ' ')|| --UNDL_TRSFR_AMT -> 전송제외로 변경
            ...
            (
                WITH BATCH_PARAM AS (
                    SELECT
                        ...
                        '28' AS GEUMGO_CODE,
                        '20250228' AS BAS_DT,
                        TO_NUMBER('2025' - 1) AS  BEF_HOIKYE_YR,
                    ...
                    FROM DUAL
                         ...
                         FROM
                             (
                                 ...
                                 WHERE 1=1
                                   ...
                                   AND ('' IS NULL OR '' = B.ICH_SIGUMGO_GUN_GU_C)
                     ...
                     GYEJWA AS (
                         ...
                         WHERE 1=1
                           AND ('' IS NULL OR '' = E.CMM_DTL_C)
                           AND ('' IS NULL OR '' = C.ACNT_DV_CD)
                           AND ('' IS NULL OR '' = B.ICH_SIGUMGO_GUN_GU_C)
                           AND C.USE_YN = 'Y'
                     ...
                     GEORAE AS (
                         ...
                         FROM (
                                  ...
                                  WHERE 1=1
                                    AND A.SIGUMGO_ORG_C = D.GEUMGO_CODE
                                    AND ('' IS NULL OR '' = A.ICH_SIGUMGO_GUN_GU_C)
                ...
                SELECT
                    ...
                    D.BAS_DT AS ACNT_YMD
                     ...
                     , SUM(CASE WHEN A.HRNK_CMM_C_NM = '세입' THEN A.TRAMT ELSE 0 END) + NVL(C.GONGGEUM_JANAEK,0) + NVL(C.UNYONG_JANAEK,0) AS NU_TXRV_DD_SUM_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_DTL_C = '수납' THEN A.TRAMT ELSE 0 END) + NVL(C.GONGGEUM_JANAEK,0) + NVL(C.UNYONG_JANAEK,0) AS NU_RCVMT_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_DTL_C = '과오납환급' THEN -1 * A.TRAMT ELSE 0 END) AS NU_RFN_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_DTL_C = '과목경정' THEN A.TRAMT ELSE 0 END) AS NU_TXRV_SBJ_RVSN_AMT
                     , 0 AS NU_FDAC_RTN_AMT
                     , 0 AS NU_FDAC_RAMT_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_C_NM = '세출' THEN -1 * A.TRAMT ELSE 0 END) AS NU_ANE_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_DTL_C = '지출' THEN -1 * A.TRAMT ELSE 0 END) AS NU_EP_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_DTL_C = '반납' THEN A.TRAMT ELSE 0 END) AS NU_RTN_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_DTL_C = '세출과목경정' THEN -1 * A.TRAMT ELSE 0 END) AS NU_ANE_SBJ_RVSN_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_DTL_C = '자금배정' THEN -1 * A.TRAMT ELSE 0 END) AS NU_FDAC_AMT
                     ,0 AS NU_PMOD_NPYM_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_C_NM = '자금운용' THEN -1 * A.TRAMT ELSE 0 END)  + NVL(C.UNYONG_JANAEK,0) AS NU_FD_OP_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_DTL_C = '자금운용예치' THEN -1 * A.TRAMT ELSE 0 END) + NVL(C.UNYONG_JANAEK,0) AS NU_FD_OP_DPI_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_DTL_C = '자금운용해지' THEN A.TRAMT ELSE 0 END) AS NU_FD_OP_CNLT_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_DTL_C = '자금전용' THEN A.TRAMT ELSE 0 END) AS NU_FD_BDTR_AMT
                     , SUM(CASE WHEN A.HRNK_CMM_C_NM = '지급명령미지급액' THEN A.TRAMT ELSE 0 END) AS NU_UNDL_TRSFR_AMT
                     , 0 AS STBX_RAMT_AMT
                     , 0 AS NU_STBX_RAMT_AMT
                     , '10' AS DAY_MON_FG
                     , '088' STBX_BANK_CD
                FROM  GYEJWA C
                          LEFT OUTER JOIN GEORAE A ON C.GONGGEUM_GYEJWA = A.GONGGEUM_GYEJWA AND C.FYR = A.SIGUMGO_HOIKYE_YR
                          INNER JOIN BATCH_PARAM D ON 1=1
                WHERE 1=1
