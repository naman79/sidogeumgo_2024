...
select
    ...
    TRXNO,
    LINKAC_JAN,
    DECODE(CRT_CAN_G,1,-1,2,-1,33,-1,1) * DECODE(IPJI_G,2,-1,1) * TRAMT AS TRAMT
from acl_sigumgo_slv
where 1=1
and fil_100_ctnt5 = '13000080900000199'
order by trxdt desc , trxno desc
...
FROM  (
        ...
        FROM  (
                          SELECT  A.TRXDT  KIJUNIL,    /*  공금거래  입지  발췌  */
                                        ...
                                        A.SIGUMGO_ORG_C  GEUMGO_CODE
                          FROM  ACL_SIGUMGO_SLV  A
                          WHERE  A.FIL_100_CTNT5  =  '13000080900000199'
                              AND  A.TRXDT>=  '20070205'
                          UNION  ALL
                          SELECT  A.DW_BAS_DDT  KIJUNIL,
                                        '13000080900000199'  AS  GONGGEUM_GYEJWA,
                                        0  IPGEUM_AMT,
                                        0  JIGEUB_AMT,
                                        0  BFJAN,
                                        130  AS  GEUMGO_CODE
                          FROM  MAP_JOB_DATE  A
                          WHERE  A.DW_BAS_DDT  BETWEEN  '20140102'  AND  TO_CHAR(SYSDATE,  'YYYYMMDD')
                          UNION  ALL  /*  신규  후  거래가  하나도  없어도  잔액이  생성될  수  있도록  0원  잔액  추가  */
                          SELECT  TO_CHAR(SYSDATE,  'YYYYMMDD')  KIJUNIL,
                                        '13000080900000199'  AS  GONGGEUM_GYEJWA,
                                        0  IPGEUM_AMT,
...
WHERE  A.GONGGEUM_GYEJWA  =  '13000080900000199'
    ...
    INNER JOIN (
        ...
        FROM
            ACL_SIGUMGO_SLV
        WHERE
            FIL_100_CTNT5 = '13000080900000199'
        GROUP BY FIL_100_CTNT5, TRXDT
    ) B
    ON A.FIL_100_CTNT5 = B.FIL_100_CTNT5
       AND A.TRXDT = B.TRXDT
       AND A.TRXNO = B.MAX_TRXNO
    WHERE
        A.FIL_100_CTNT5 = '13000080900000199'
)
SELECT
    KEORAEIL,
    MAX(CASE WHEN SOURCE = 'RPT_GONGGEUM_JAN' THEN JANAEK ELSE 0 END) AS RPT_JANAEK,
...
-- HNDO_TRX_YN = 0 조건을 추가해서 잔액을 계산해 보자
...
WITH CombinedData AS (
    ...
    SELECT
              ...
              SUM(A.BFJAN  +  A.IPGEUM_AMT  -  A.JIGEUB_AMT)
                      OVER  (PARTITION  BY  A.GONGGEUM_GYEJWA  ORDER  BY  A.KIJUNIL)  JANEAK,
                'RPT_GONGGEUM_JAN_HDNO_TRX_YN' AS SOURCE
FROM  (
        SELECT  A.KIJUNIL,
                      A.GONGGEUM_GYEJWA,
                      SUM(A.IPGEUM_AMT)  IPGEUM_AMT,
...
SELECT
    KEORAEIL,
    MAX(CASE WHEN SOURCE = 'RPT_GONGGEUM_JAN' THEN JANAEK ELSE 0 END) AS RPT_JANAEK,
    MAX(CASE WHEN SOURCE = 'RPT_GONGGEUM_JAN_HDNO_TRX_YN' THEN JANAEK ELSE 0 END) AS RPT_GONGGEUM_JAN_HDNO_TRX_YN,
    MAX(CASE WHEN SOURCE = 'ACL_SIGUMGO_SLV' THEN JANAEK ELSE 0 END) AS SIGUMGO_JANAEK,
    (MAX(CASE WHEN SOURCE = 'RPT_GONGGEUM_JAN' THEN JANAEK ELSE 0 END) - MAX(CASE WHEN SOURCE = 'ACL_SIGUMGO_SLV' THEN JANAEK ELSE 0 END)) AS DIFF1,
    (MAX(CASE WHEN SOURCE = 'RPT_GONGGEUM_JAN_HDNO_TRX_YN' THEN JANAEK ELSE 0 END) - MAX(CASE WHEN SOURCE = 'ACL_SIGUMGO_SLV' THEN JANAEK ELSE 0 END)) AS DIFF2
FROM
    CombinedData
GROUP BY KEORAEIL
ORDER BY
...
WITH CombinedData AS (
    ...
    SELECT
        ...
        'RPT_GONGGEUM_JAN' AS SOURCE
    FROM RPT_GONGGEUM_JAN
    WHERE GONGGEUM_GYEJWA = '02800001210000024'
...
FROM  (
        ...
        FROM  (
                          SELECT  NVL(A.GISDT,A.TRXDT)  KIJUNIL,    /*  공금거래  입지  발췌  */
                                        ...
                                        A.SIGUMGO_ORG_C  GEUMGO_CODE
                          FROM  ACL_SIGUMGO_SLV  A
                          WHERE  A.FIL_100_CTNT5  =  '02800001210000024'
                              AND HNDO_TRX_YN = 0
                              AND  A.TRXDT>=  '20070205'
                          UNION  ALL
                          SELECT  A.DW_BAS_DDT  KIJUNIL,
                                        '02800001210000024'  AS  GONGGEUM_GYEJWA,
                                        0  IPGEUM_AMT,
                                        0  JIGEUB_AMT,
                                        0  BFJAN,
                                        28  AS  GEUMGO_CODE
...
WHERE  A.GONGGEUM_GYEJWA  =  '02800001210000024'
    AND  A.GEUMGO_CODE    =  28
    ...
    INNER JOIN (
        ...
        FROM
            ACL_SIGUMGO_SLV
        WHERE
            FIL_100_CTNT5 = '02800001210000024'
        GROUP BY FIL_100_CTNT5, TRXDT
    ) B
    ON A.FIL_100_CTNT5 = B.FIL_100_CTNT5
       AND A.TRXDT = B.TRXDT
       AND A.TRXNO = B.MAX_TRXNO
    WHERE
        A.FIL_100_CTNT5 = '02800001210000024'
)
SELECT
    KEORAEIL,
    MIN(CASE WHEN SOURCE = 'RPT_GONGGEUM_JAN' THEN JANAEK ELSE 0 END) AS RPT_JANAEK,
    MIN(CASE WHEN SOURCE = 'RPT_GONGGEUM_JAN_HDNO_TRX_YN' THEN JANAEK ELSE 0 END) AS RPT_GONGGEUM_JAN_HDNO_TRX_YN,
    MIN(CASE WHEN SOURCE = 'ACL_SIGUMGO_SLV' THEN JANAEK ELSE 0 END) AS SIGUMGO_JANAEK,
    (MIN(CASE WHEN SOURCE = 'RPT_GONGGEUM_JAN' THEN JANAEK ELSE 0 END) - MIN(CASE WHEN SOURCE = 'ACL_SIGUMGO_SLV' THEN JANAEK ELSE 0 END)) AS DIFF1,
    (MIN(CASE WHEN SOURCE = 'RPT_GONGGEUM_JAN_HDNO_TRX_YN' THEN JANAEK ELSE 0 END) - MIN(CASE WHEN SOURCE = 'ACL_SIGUMGO_SLV' THEN JANAEK ELSE 0 END)) AS DIFF2
FROM
    CombinedData
GROUP BY KEORAEIL
ORDER BY
...
WITH CombinedData AS (
    ...
    SELECT
        ...
        'RPT_GONGGEUM_JAN' AS SOURCE
    FROM RPT_GONGGEUM_JAN
    WHERE GONGGEUM_GYEJWA = '02811001250004724'
...
FROM  (
        ...
        FROM  (
                          SELECT  NVL(A.GISDT,A.TRXDT)  KIJUNIL,    /*  공금거래  입지  발췌  */
                                        ...
                                        A.SIGUMGO_ORG_C  GEUMGO_CODE
                          FROM  ACL_SIGUMGO_SLV  A
                          WHERE  A.FIL_100_CTNT5  =  '02811001250004724'
                              AND HNDO_TRX_YN = 0
                              AND  A.TRXDT>=  '20070205'
                          UNION  ALL
                          SELECT  A.DW_BAS_DDT  KIJUNIL,
                                        '02811001250004724'  AS  GONGGEUM_GYEJWA,
                                        0  IPGEUM_AMT,
                                        0  JIGEUB_AMT,
                                        0  BFJAN,
                                        28  AS  GEUMGO_CODE
...
WHERE  A.GONGGEUM_GYEJWA  =  '02811001250004724'
    AND  A.GEUMGO_CODE    =  28
    ...
    INNER JOIN (
        ...
        FROM
            ACL_SIGUMGO_SLV
        WHERE
            FIL_100_CTNT5 = '02811001250004724'
        GROUP BY FIL_100_CTNT5, TRXDT
    ) B
    ON A.FIL_100_CTNT5 = B.FIL_100_CTNT5
       AND A.TRXDT = B.TRXDT
       AND A.TRXNO = B.MAX_TRXNO
    WHERE
        A.FIL_100_CTNT5 = '02811001250004724'
)
SELECT
    KEORAEIL,
    MIN(CASE WHEN SOURCE = 'RPT_GONGGEUM_JAN' THEN JANAEK ELSE 0 END) AS RPT_JANAEK,
