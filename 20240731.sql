...
SELECT
 CASE 
    WHEN B.SIGUMGO_HOIKYE_YR <> 9999 THEN SUBSTR(A.GONGGEUM_GYEJWA, 0, 15) || SUBSTR((SELECT BIZ_DT FROM MAP_JOB_DATE WHERE DW_BAS_DDT = '20240731'), 3, 2) 
...
WHERE 1=1
AND B.MNG_NO = 1
AND A.KIJUNIL = (SELECT CASE WHEN DT_G = 0 THEN BIZ_DT ELSE BF1_BIZ_DT END AS BIZ_DT FROM MAP_JOB_DATE WHERE DW_BAS_DDT = '20231231')
AND A.JANAEK > 0
ORDER BY A.GONGGEUM_GYEJWA, A.UNYONG_GYEJWA

---------------------------------------------------------------------------------------------

SELECT * FROM MAP_JOB_DATE WHERE DW_BAS_DDT LIKE '%1231'
...
FROM ACL_SIGUMGO_MAS
WHERE MNG_NO = 1
AND (HJI_DT IS NULL OR HJI_DT <= '20241231')

-----------------------------------------------------------------------------------------------

SELECT  * FROM rpt_sunap_jibgye
WHERE 1=1 
AND UPD_DTTM LIKE '20240801%'
AND keoraeil <> '20240801'
AND SUNAPIL NOT IN (
SELECT SUNAPIL FROM rpt_sunap_jibgye
WHERE 1=1 
AND UPD_DTTM LIKE '20240801%'
AND keoraeil = '20240801'
)