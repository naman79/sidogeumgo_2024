SELECT
     T2.HOIGYE_CODE
    ,T2.GONGGEUM_GYEJWA
    ,T2.GONGGEUM_GYEJWA_NM
    ,NVL(TRUNC(SUM(T1.GONGGEUM)/MAX(T3.DAYS_CNT), 0), 0) AS GONGGEUM
    ,NVL(TRUNC(SUM(T1.UNYONG)/MAX(T3.DAYS_CNT), 0), 0) AS JUCHUK
    ,NVL(TRUNC(SUM(T1.MMDA)/MAX(T3.DAYS_CNT), 0), 0) AS MMDA
    ,T2.GONGGEUM_YUDONG_ACNO
FROM
     (
      SELECT
           T1.HOIGYE_CODE
          ,T1.GONGGEUM_GYEJWA
          ,T1.TRX_DT
          ,T1.BIZ_DT
          ,T1.JANAEK AS GONGGEUM
          ,0 AS UNYONG
          ,0 AS MMDA
      FROM
           (
            SELECT
                 T1.HOIGYE_CODE
                ,T1.GONGGEUM_GYEJWA
                ,T2.DW_BAS_DDT AS TRX_DT
                ,DECODE(T2.DT_G, 0, T2.BIZ_DT, T2.BF1_BIZ_DT) AS BIZ_DT
                ,T1.JANAEK
            FROM
                 RPT_GONGGEUM_JAN T1
                ,MAP_JOB_DATE T2
            WHERE 1=1
                 AND T1.KEORAEIL = DECODE(T2.DT_G, 0, T2.BIZ_DT, T2.BF1_BIZ_DT)
                 AND T1.GEUMGO_CODE = '42'
                 AND T1.GUNGU_CODE = 0
                 AND T2.DW_BAS_DDT >= '20240101'
                 AND T2.DW_BAS_DDT <= '20240630'
           ) T1
          ,ACL_SIGUMGO_MAS T2
          ,RPT_AC_BY_HOIKYE_MAPP T3
      WHERE 1=1
           AND T1.GONGGEUM_GYEJWA = T2.FIL_100_CTNT2
           AND T2.FIL_100_CTNT2 = T3.SIGUMGO_ACNO
           AND T2.SIGUMGO_HOIKYE_YR = T3.SIGUMGO_HOIKYE_YR
           AND T2.MNG_NO = 1
           AND (
                CASE
                     WHEN T2.SIGUMGO_HOIKYE_YR = '9999' AND SUBSTR(T1.TRX_DT, 1, 4) = '2024'
                     THEN 1
                     WHEN T2.SIGUMGO_HOIKYE_YR = '2024'
                     THEN 1
                     ELSE 0
                  END
               ) = 1
      UNION ALL
      SELECT
           T1.HOIGYE_CODE
          ,T1.GONGGEUM_GYEJWA
          ,T1.TRX_DT
          ,T1.BIZ_DT
          ,0 AS GONGGEUM
          ,CASE
               WHEN '2024' > '2021'
               THEN
                    CASE
                        WHEN T1.TRX_DT >= NVL(T2.KIJUNIL, '19000101') THEN NVL(T1.JANAEK, 0)
                        ELSE 0
                     END
               ELSE NVL(T1.JANAEK, 0)
            END AS UNYONG
          ,0 AS MMDA
      FROM
           (
            SELECT
                 TO_NUMBER(T3.SIGUMGO_HOIKYE_C) AS HOIGYE_CODE
                ,T2.FIL_100_CTNT2 AS GONGGEUM_GYEJWA
                ,T1.TRX_DT
                ,T1.BIZ_DT
                ,T1.JANAEK
                ,T2.SIGUMGO_HOIKYE_YR AS HOIGYE_YEAR
                ,CASE
                     WHEN T2.SIGUMGO_HOIKYE_YR = '9999' AND T1.GONGGEUM_GYEJWA = T2.FIL_100_CTNT2
                     THEN 'Y'
                     WHEN T2.SIGUMGO_HOIKYE_YR = '2024' AND T1.GONGGEUM_GYEJWA <= T2.FIL_100_CTNT2
                     THEN 'Y'
                     ELSE 'N'
                  END AS TRGT_YN
            FROM
                 (
                  SELECT
                       T2.GONGGEUM_GYEJWA
                      ,SUBSTR(T2.GONGGEUM_GYEJWA, 1, 15) AS GONGGEUM_GYEJWA_15
                      ,T1.JANAEK
                      ,T1.TRX_DT
                      ,T1.BIZ_DT
                      ,CASE
                           WHEN     T1.TRX_DT >= SUBSTR(NVL(T2.MKDT,T2.IN_DATE),1,8) 
                                AND T1.TRX_DT < NVL(T2.OUT_DATE, '99991231')
                                AND T1.TRX_DT < NVL(T2.HJI_DT, '99991231')
                           THEN 'Y'
                           ELSE 'N'
                        END AS TRGT_YN
                  FROM
                       (
                        SELECT
                             T1.GONGGEUM_GYEJWA
                            ,T1.UNYONG_GYEJWA
                            ,T2.DW_BAS_DDT AS TRX_DT
                            ,DECODE(T2.DT_G, 0, T2.BIZ_DT, T2.BF1_BIZ_DT) AS BIZ_DT
                            ,T1.JANAEK
                        FROM
                             RPT_UNYONG_JAN T1
                            ,MAP_JOB_DATE T2
                        WHERE 1=1
                             AND T1.KIJUNIL = DECODE(T2.DT_G, 0, T2.BIZ_DT, T2.BF1_BIZ_DT)
                             AND T1.GEUMGO_CODE = '42'
                             AND T2.DW_BAS_DDT >= '20240101'
                             AND T2.DW_BAS_DDT <= '20240630'
                       ) T1
                      ,RPT_UNYONG_GYEJWA T2
                      ,(
                        SELECT
                             T1.FIL_100_CTNT2 AS GONGGEUM_GYEJWA
                            ,T2.KIJUNIL
                        FROM
                             ACL_SIGUMGO_MAS T1
                            ,(
                              SELECT
                                   T1.SIGUMGO_ORG_C
                                  ,T1.GONGGEUM_GYEJWA
                                  ,T1.ICH_SIGUMGO_GUN_GU_C
                                  ,T1.ICH_SIGUMGO_HOIKYE_C
                                  ,T1.SIGUMGO_AC_B
                                  ,T1.SIGUMGO_AC_SER
                                  ,T1.SIGUMGO_HOIKYE_YR
                                  ,T2.KIJUNIL
                              FROM
                                   (
                                    SELECT
                                         T1.SIGUMGO_ORG_C
                                        ,T1.FIL_100_CTNT2 AS GONGGEUM_GYEJWA
                                        ,T1.ICH_SIGUMGO_GUN_GU_C
                                        ,T1.ICH_SIGUMGO_HOIKYE_C
                                        ,T1.SIGUMGO_AC_B
                                        ,T1.SIGUMGO_AC_SER
                                        ,T1.SIGUMGO_HOIKYE_YR
                                    FROM
                                         ACL_SIGUMGO_MAS T1
                                    WHERE 1=1
                                         AND T1.SIGUMGO_ORG_C = '42'
                                         AND T1.SIGUMGO_HOIKYE_YR = '2024'
                                         AND T1.MNG_NO = 1
                                   ) T1
                                  ,RPT_HOIGYE_IWOL T2
                              WHERE 1=1
                                   AND T1.SIGUMGO_ORG_C = T2.GEUMGO_CODE
                                   AND T1.ICH_SIGUMGO_GUN_GU_C = T2.GUNGU_CODE
                                   AND T1.SIGUMGO_HOIKYE_YR = TO_CHAR(T2.HOIGYE_YEAR + 1)
                                   AND T2.KIJUNIL <= '20240630'
                                   AND T2.HOIGYE_CODE IN (
                                                          SELECT
                                                               TO_NUMBER(T3.SIGUMGO_HOIKYE_C)
                                                          FROM
                                                               RPT_AC_BY_HOIKYE_MAPP T3
                                                          WHERE 1=1
                                                               AND T3.SIGUMGO_ACNO = T1.GONGGEUM_GYEJWA
                                                         )
                                   
                             ) T2
                        WHERE 1=1
                             AND T1.SIGUMGO_ORG_C = T2.SIGUMGO_ORG_C
                             AND T1.ICH_SIGUMGO_GUN_GU_C = T2.ICH_SIGUMGO_GUN_GU_C
                             AND T1.ICH_SIGUMGO_HOIKYE_C = T2.ICH_SIGUMGO_HOIKYE_C
                             AND T1.SIGUMGO_AC_B = T2.SIGUMGO_AC_B
                             AND T1.SIGUMGO_AC_SER = T2.SIGUMGO_AC_SER
                             AND T1.SIGUMGO_HOIKYE_YR < T2.SIGUMGO_HOIKYE_YR
                             AND T1.SIGUMGO_ORG_C = '42'
                             AND T1.MNG_NO = 1
                        UNION
                        SELECT
                             T1.FIL_100_CTNT2 AS GONGGEUM_GYEJWA
                            ,'19000101' AS KIJUNIL
                        FROM
                             ACL_SIGUMGO_MAS T1
                        WHERE 1=1
                             AND T1.SIGUMGO_ORG_C = '42'
                             AND T1.MNG_NO = 1
                             AND (
                                  CASE
                                      WHEN T1.SIGUMGO_HOIKYE_YR = '9999' AND SUBSTR('20240630', 1, 4) = '2024'
                                      THEN 1
                                      WHEN T1.SIGUMGO_HOIKYE_YR = '2024'
                                      THEN 1
                                      ELSE 0
                                   END
                                 ) = 1
                       ) T3
                  WHERE 1=1
                       AND T1.GONGGEUM_GYEJWA = T2.GONGGEUM_GYEJWA
                       AND T1.UNYONG_GYEJWA = T2.UNYONG_GYEJWA
                       AND T1.GONGGEUM_GYEJWA = T3.GONGGEUM_GYEJWA
                       AND T1.TRX_DT >= T3.KIJUNIL
                       AND T2.BANK_GUBUN = 0
                 ) T1
                ,ACL_SIGUMGO_MAS T2
                ,RPT_AC_BY_HOIKYE_MAPP T3
      WHERE 1=1
           AND T2.FIL_100_CTNT2 LIKE T1.GONGGEUM_GYEJWA_15||'%'
           AND T2.FIL_100_CTNT2 = T3.SIGUMGO_ACNO
           AND T2.SIGUMGO_HOIKYE_YR = T3.SIGUMGO_HOIKYE_YR
           AND T1.TRGT_YN = 'Y'
           AND T2.MNG_NO = 1
      ) T1
     ,(
       SELECT
            T1.KIJUNIL
           ,T1.HOIGYE_YEAR
           ,T1.HOIGYE_CODE
           ,T1.GUNGU_CODE
           ,T1.GEUMGO_CODE
           ,SUM(T1.AMT1) AS AMT2
       FROM
            RPT_HOIGYE_IWOL T1
       WHERE 1=1
            AND '2024' > '2021'
            AND T1.HOIGYE_YEAR = '2024'
            AND T1.GEUMGO_CODE = '42'
            AND T1.GUNGU_CODE = 0
            AND GUBUN_CODE = 2
       GROUP BY
            T1.KIJUNIL
           ,T1.HOIGYE_YEAR
           ,T1.HOIGYE_CODE
           ,T1.GUNGU_CODE
           ,T1.GEUMGO_CODE
       HAVING SUM(T1.AMT1) > 0 
      ) T2
      WHERE 1=1
            AND T1.HOIGYE_CODE = T2.HOIGYE_CODE(+)
            AND T1.HOIGYE_YEAR = T2.HOIGYE_YEAR(+)
            AND T1.TRGT_YN = 'Y'
      UNION ALL
      SELECT
           T1.GONGGEUM_HOIGYE_CODE
          ,T1.GONGGEUM_GYEJWA
          ,T1.TRX_DT
          ,T1.BIZ_DT
          ,0 AS GONGGEUM
          ,0 AS UNYONG
          ,T2.JANAEK AS MMDA
      FROM
           (
            SELECT DISTINCT
                 T1.FIL_100_CTNT2 AS GONGGEUM_GYEJWA
                ,TO_NUMBER(T2.SIGUMGO_HOIKYE_C) AS GONGGEUM_HOIGYE_CODE
                ,T3.TRX_DT
                ,T3.BIZ_DT
                ,T3.DAYS_CNT
            FROM
                 ACL_SIGUMGO_MAS T1
                ,RPT_AC_BY_HOIKYE_MAPP T2
                ,(
                  SELECT
                        T1.DW_BAS_DDT AS TRX_DT
                       ,DECODE(T1.DT_G, 0, T1.BIZ_DT, T1.BF1_BIZ_DT) AS BIZ_DT
                       ,COUNT(1) OVER() AS DAYS_CNT
                  FROM
                       MAP_JOB_DATE T1
                  WHERE 1=1
                       AND T1.DW_BAS_DDT >= '20240101'
                       AND T1.DW_BAS_DDT <= '20240630'
                 ) T3
            WHERE 1=1
                 AND T1.FIL_100_CTNT2 = T2.SIGUMGO_ACNO
                 AND T1.SIGUMGO_HOIKYE_YR = T2.SIGUMGO_HOIKYE_YR
                 AND T1.SIGUMGO_ORG_C = '42'
                 AND (
                      CASE
                          WHEN T1.SIGUMGO_HOIKYE_YR = '9999' AND SUBSTR(T3.TRX_DT, 1, 4) = '2024'
                          THEN 1
                          WHEN T1.SIGUMGO_HOIKYE_YR = '2024'
                          THEN 1
                          ELSE 0
                       END
                     ) = 1
           ) T1
          ,RPT_UNYONG_JAN T2
      WHERE 1=1
           AND T1.GONGGEUM_GYEJWA = T2.GONGGEUM_GYEJWA
           AND T1.BIZ_DT = T2.KIJUNIL
           AND T2.UNYONG_GYEJWA = '000000000000'
      UNION ALL
      SELECT
          T1.HOIGYE_CODE
         ,T1.GONGGEUM_GYEJWA
         ,T1.TRX_DT
         ,T1.BIZ_DT
         ,0 AS GONGGEUM
         ,T2.JANAEK AS UNYONG
         ,0 AS MMDA
     FROM
          (
          SELECT
               T2.GONGGEUM_GYEJWA
              ,T2.UNYONG_GYEJWA
              ,T1.HOIGYE_CODE
              ,T1.TRX_DT
              ,T1.BIZ_DT
              ,T1.DAYS_CNT
          FROM
               (
               SELECT
                    T1.REF_D_C + 900 AS HOIGYE_CODE
                   ,T1.REF_D_NM AS GONGGEUM_GYEJWA
                   ,T2.TRX_DT
                   ,T2.BIZ_DT
                   ,T2.DAYS_CNT
               FROM
                    RPT_CODE_INFO T1
                   ,(
                     SELECT
                           T1.DW_BAS_DDT AS TRX_DT
                          ,DECODE(T1.DT_G, 0, T1.BIZ_DT, T1.BF1_BIZ_DT) AS BIZ_DT
                          ,COUNT(1) OVER() AS DAYS_CNT
                     FROM
                          MAP_JOB_DATE T1
                     WHERE 1=1
                          AND T1.DW_BAS_DDT >= '20240101'
                          AND T1.DW_BAS_DDT <= '20240630'
                    ) T2
               WHERE 1=1
                    AND T1.REF_L_C =50
                    AND T1.REF_M_C = '42'
                    AND T1.YUHYO_YN = 0
               ) T1
              ,RPT_UNYONG_GYEJWA T2
          WHERE 1=1
               AND T1.GONGGEUM_GYEJWA = T2.GONGGEUM_GYEJWA
               AND T1.TRX_DT >= SUBSTR(NVL(T2.MKDT,T2.IN_DATE),1,8)
               AND T1.TRX_DT < NVL(T2.OUT_DATE, '99991231')
               AND T1.TRX_DT < NVL(T2.HJI_DT, '99991231')
               AND SUBSTR(T1.TRX_DT, 1, 4) = '2024'
               AND T2.BANK_GUBUN = 0
          ) T1
         ,RPT_UNYONG_JAN T2
     WHERE 1=1
          AND T1.GONGGEUM_GYEJWA = T2.GONGGEUM_GYEJWA
          AND T1.UNYONG_GYEJWA = T2.UNYONG_GYEJWA
          AND T1.BIZ_DT = T2.KIJUNIL
     ) T1
    ,(
      SELECT
           TO_NUMBER(T2.SIGUMGO_HOIKYE_C) AS HOIGYE_CODE
          ,T1.FIL_100_CTNT2 AS GONGGEUM_GYEJWA
          ,T1.SIGUMGO_AC_NM AS GONGGEUM_GYEJWA_NM
          ,T1.SIGUMGO_HOIKYE_YR AS GONGGEUM_HOIGYE_YEAR
          ,LPAD(T1.LINKAC_KWA_C, 3, '0')||LPAD(T1.LINK_ACSER, 9, '0') AS GONGGEUM_YUDONG_ACNO
      FROM
           ACL_SIGUMGO_MAS T1
          ,RPT_AC_BY_HOIKYE_MAPP T2
      WHERE 1=1
           AND T1.FIL_100_CTNT2 = T2.SIGUMGO_ACNO
           AND T1.SIGUMGO_HOIKYE_YR = T2.SIGUMGO_HOIKYE_YR
           AND T1.SIGUMGO_ORG_C = '42'
           AND T1.MNG_NO = 1
           AND T1.FIL_100_CTNT2 <> '04200080900003999'
           AND (
                CASE
                    WHEN '' IS NULL
                    THEN 
                         CASE
                             WHEN T1.SIGUMGO_HOIKYE_YR IN ('2024', '9999')
                             THEN 1
                             ELSE 0
                          END
                    ELSE DECODE(T1.SIGUMGO_HOIKYE_YR, '2024', 1, 0)
                 END
               ) = 1
     ) T2
    ,(
      SELECT
            COUNT(1) AS DAYS_CNT
      FROM
           MAP_JOB_DATE T1
      WHERE 1=1
           AND T1.DW_BAS_DDT >= '20240101'
           AND T1.DW_BAS_DDT <= '20240630'
     ) T3
WHERE 1=1
     AND T1.GONGGEUM_GYEJWA(+) = T2.GONGGEUM_GYEJWA
   AND T2.HOIGYE_CODE  IN ( 
   
     SELECT T2.SIGUMGO_HOIKYE_C
     FROM
     RPT_HOIKYE_GRP_INFO T1
    ,RPT_HOIKYE_GRP_HOIKYE_CD T2
    WHERE 1=1
    AND T1.SIGUMGO_C = T2.SIGUMGO_C
    AND T1.HOIKYE_GRP_ID = T2.HOIKYE_GRP_ID
    AND T1.SIGUMGO_C = '42'
    AND  T1.HOIKYE_GRP_ID = '1003'
     
         )
   
GROUP BY
     T2.HOIGYE_CODE
    ,T1.GONGGEUM_GYEJWA
    ,T2.GONGGEUM_GYEJWA
    ,T2.GONGGEUM_GYEJWA_NM
    ,T2.GONGGEUM_HOIGYE_YEAR
    ,T2.GONGGEUM_YUDONG_ACNO
ORDER BY
    T2.HOIGYE_CODE
    ,T1.GONGGEUM_GYEJWA
    ,T2.GONGGEUM_GYEJWA
    ,T2.GONGGEUM_GYEJWA_NM
    ,T2.GONGGEUM_HOIGYE_YEAR
    ,T2.GONGGEUM_YUDONG_ACNO


------------------------------------------------------------------------

select * from acl_sigyul_slv
where gjdt like '202407%'

select * from RPT_CODE_INFO where REF_L_C = 200
and REF_S_NM like '2024%'


select * from ACL_SIGUMGO_SLV
where 1=1
and CRT_CAN_G not in (0, 1)


-- 04200080900000099   20240228   160000133730 
-- 04200080900002499   20240313   160000114617 
-- 04200080900002999   20240513   160000116081
-- 04200080900003999   20240228   160000125420
-- 04200080900003999   20240513   160000125420
-- 04200080900004199   20240620   160000128110

-- 수정대상 총 6건이고 잔액이 맞는진 좀 더 검토가 필요합니다.
-- CRT_CAN_G는 모두 31로 들어왔고요

-- 
select * from 
rpt_unyong_jan
where 1=1
and gonggeum_gyejwa = '04200080900000099'
and unyong_gyejwa = '000000000000'
and KIJUNIL like '202402%'
order by KIJUNIL

-- 
select * from 
rpt_unyong_jan
where 1=1
and gonggeum_gyejwa = '04200080900002499'
and unyong_gyejwa = '000000000000'
and KIJUNIL like '202403%'
order by KIJUNIL

-- 처리되어 있음
select * from 
rpt_unyong_jan
where 1=1
and gonggeum_gyejwa = '04200080900002999'
and unyong_gyejwa = '000000000000'
and KIJUNIL like '202405%'
order by KIJUNIL

select * from ACL_SIGUMGO_SLV
where 1=1
and FIL_100_CTNT5 = '04200080900002999'
and TRXDT = '20240513'
and SIGUMGO_TRX_G in (40, 90)

-- 보정필요
select * from 
rpt_unyong_jan
where 1=1
and gonggeum_gyejwa = '04200080900003999'
and unyong_gyejwa = '000000000000'
and KIJUNIL like '202402%'
order by KIJUNIL

-- jeonjan: 17,121,088,731
-- kijunil: 20240228 ip: 1,600,000,000 ji: 0 jan: 18,721,088,731
select * from ACL_SIGUMGO_SLV
where 1=1
and FIL_100_CTNT5 = '04200080900003999'
and TRXDT = '20240228'
and SIGUMGO_TRX_G in (40, 90)

UPDATE RPT_UNYONG_JAN
SET JANAEK = 18721088731
, IPAMT = 1600000000
, JIAMT = 0
WHERE 1=1
AND KIJUNIL = '20240228'
AND GONGGEUM_GYEJWA = '04200080900003999'
AND UNYONG_GYEJWA = '000000000000'


-- 이미 보정처리 완료됨(언젠가 수동으로?)
select * from 
rpt_unyong_jan
where 1=1
and gonggeum_gyejwa = '04200080900003999'
and unyong_gyejwa = '000000000000'
and KIJUNIL like '202405%'
order by KIJUNIL

select * from ACL_SIGUMGO_SLV
where 1=1
and FIL_100_CTNT5 = '04200080900003999'
and TRXDT = '20240513'

-- 이미 보정처리 완료됨
select * from 
rpt_unyong_jan
where 1=1
and gonggeum_gyejwa = '04200080900004199'
and unyong_gyejwa = '000000000000'
and KIJUNIL like '202406%'
order by KIJUNIL

select * from ACL_SIGUMGO_SLV
where 1=1
and FIL_100_CTNT5 = '04200080900004199'
and TRXDT = '20240620'

-----------------------------------

SELECT * FROM 
MAP_JOB_DATE
WHERE 1=1


SELECT * FROM 
MAP_JOB_DATE
WHERE 1=1
AND DW_BAS_DDT = '20240313'

SELECT * FROM 
MAP_JOB_DATE
WHERE 1=1
AND DW_BAS_DDT = '20240314'
-----------------------------------

SELECT A.DW_BAS_DDT, A.GONGGEUM_GYEJWA, '000000000000' UNYONG_GYEJWA, NVL(B.JANAEK,0) JANAEK,
NVL(B.MKDT, '') MKDT, NVL(B.DUDT, '') DUDT, NVL(B.HJI_DT, '') HJI_DT,
NVL(B.BBK_BKKP_NM, '') BBK_BKKP_NM, NVL(B.PRDT_BKKP_NM,'') PRDT_BKKP_NM,
'20240314' LST_TRXDT, NVL(B.IYUL, '') IYUL, A.GEUMGO_CODE, 0 IPAMT, 0 JIAMT, 0JI_IJA 
FROM
(
SELECT DISTINCT A.GONGGEUM_GYEJWA, D.DW_BAS_DDT, D.BIZ_DT, D.BF1_BIZ_DT, A.GEUMGO_CODE
FROM RPT_UNYONG_JAN A, MAP_JOB_DATE D
WHERE 1=1
AND A.KIJUNIL = D.BF1_BIZ_DT
AND A.UNYONG_GYEJWA = '000000000000'
AND D.DW_BAS_DDT = '20240314'
AND A.GONGGEUM_GYEJWA = '04200080900002499'
) A, RPT_UNYONG_JAN B 
WHERE 1=1
AND A.BF1_BIZ_DT = B.KIJUNIL(+)
AND A.GONGGEUM_GYEJWA = B.GONGGEUM_GYEJWA(+)
AND B.UNYONG_GYEJWA(+) = '000000000000'


----------------------------------------
WITH UNYONG_JAN AS (
    SELECT
        '04200080900002499' AS GONGGEUM_GYEJWA,
        '20240314' AS KIJUNIL
    FROM DUAL
)
SELECT 
SUM(
    DECODE(
        B.CRT_CAN_G,
        0,
        DECODE(B.IPJI_G, '2', B.TRAMT,0),
        DECODE(B.IPJI_G, '2', -B.TRAMT, 0)
        )
    - DECODE(
        B.CRT_CAN_G,
        0,
        DECODE(B.IPJI_G, '1', B.TRAMT,0),
        DECODE(B.IPJI_G, '1', -B.TRAMT, 0)
        )
) AS JANAEK,
SUM(
    CASE WHEN A.KIJUNIL = B.GISDT THEN
    DECODE(B.CRT_CAN_G,
        0,
        DECODE(B.IPJI_G, '2', B.TRAMT,0),
        DECODE(B.IPJI_G, '2', -B.TRAMT, 0)
        )
) AS IPAMT,
SUM(
    CASE WHEN A.KIJUNIL = B.GISDT THEN
    DECODE(B.CRT_CAN_G,
        0,
        DECODE(B.IPJI_G, '1', B.TRAMT,0),
        DECODE(B.IPJI_G, '1', -B.TRAMT, 0)
        )
) AS JIAMT
FROM ACL_SIGUMGO_SLV B, UNYONG_JAN A 
WHERE 1=1
AND B.FIL_100_CTNT5 = A.GONGGEUM_GYEJWA
AND B.GISDT <= A.KIJUNIL
AND B.TRXDT = '20240314'
AND B.SIGUMGO_TRX_G IN (40, 90)
AND B.FIL_2_NO3 = 1
GROUP BY B.FIL_100_CTNT5

-----------------------------

WITH UNYONG_JAN AS (
    SELECT
        '04200080900002499 ' AS GONGGEUM_GYEJWA,
        '20240317' AS KIJUNIL
    FROM DUAL
)
SELECT 
SUM(
    DECODE(
        B.CRT_CAN_G,
        0,
        DECODE(B.IPJI_G, '2', B.TRAMT,0),
        DECODE(B.IPJI_G, '2', -B.TRAMT, 0)
        )
    - DECODE(
        B.CRT_CAN_G,
        0,
        DECODE(B.IPJI_G, '1', B.TRAMT,0),
        DECODE(B.IPJI_G, '1', -B.TRAMT, 0)
        )
) AS JANAEK,
SUM(
    CASE WHEN A.KIJUNIL = B.GISDT THEN
    DECODE(B.CRT_CAN_G,
        0,
        DECODE(B.IPJI_G, '2', B.TRAMT,0),
        DECODE(B.IPJI_G, '2', -B.TRAMT, 0)
        )
    END
) AS IPAMT,
SUM(
    CASE WHEN A.KIJUNIL = B.GISDT THEN
    DECODE(B.CRT_CAN_G,
        0,
        DECODE(B.IPJI_G, '1', B.TRAMT,0),
        DECODE(B.IPJI_G, '1', -B.TRAMT, 0)
        )
    END
) AS JIAMT
FROM ACL_SIGUMGO_SLV B, UNYONG_JAN A 
WHERE 1=1
AND B.FIL_100_CTNT5 = A.GONGGEUM_GYEJWA
AND B.GISDT <= A.KIJUNIL
-- AND B.TRXDT = '20240314'
AND B.SIGUMGO_TRX_G IN (40, 90)
-- AND B.FIL_2_NO3 = 1
GROUP BY B.FIL_100_CTNT5
