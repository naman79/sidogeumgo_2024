...
AND AA.HOIGYE_CODE(+) = BB.SIGUMGO_HOIKYE_C
  AND BB.SIGUMGO_HOIKYE_C IN(
       ...
       ,RPT_HOIKYE_GRP_HOIKYE_CD T2
        ...
        AND T1.SIGUMGO_C = '440'
        AND  T1.HOIKYE_GRP_ID = '1001'
...
-----------------------------------------------------------------------------------------
 ...
 SELECT   
    ...
    - (NVL(SUNP_AMT,0) - NVL(TAX_SUNP_IP_AMT,0) + NVL(IWOL_MIJUNGSAN,0)) AS SEIB_SUIP, 
     ...
     NVL(TAX_SUNP_IP_AMT,0) as TAX_SUNP_IP_AMT,
...
(
  ...
  (
   ...
   AND A.ICH_SIGUMGO_GUN_GU_C = '0'   
    AND CASE WHEN A.SIGUMGO_HOIKYE_YR = 9999 THEN TO_NUMBER(SUBSTR(A.BAS_DT,1,4))
                   ELSE TO_NUMBER(A.SIGUMGO_HOIKYE_YR) END  = '2024'
   AND A.BAS_DT BETWEEN '20240702' AND '20240702' 
...
UNION ALL 
     ...
     SUM(CASE WHEN GUBUN_CODE = 4 THEN AMT1 ELSE 0 END)                    
         ...
         AND A.GUBUN_CODE IN (1, 2, 4)         
           AND A.KIJUNIL BETWEEN '20240702' AND '20240702'    
         AND A.GUNGU_CODE = 0             
...
UNION ALL
 ...
 (
  ...
  AND A.ICH_SIGUMGO_GUN_GU_C = '0'  
    AND CASE WHEN A.SIGUMGO_HOIKYE_YR = 9999 THEN TO_NUMBER(SUBSTR(A.BAS_DT,1,4))
                   ELSE TO_NUMBER(A.SIGUMGO_HOIKYE_YR) END  = '2024'
  AND A.BAS_DT BETWEEN SUBSTR('20240101',0,4)||'0101' AND '20240702'
  AND A.SIGUMGO_ORG_C(+) = '42'
  AND B.MNG_NO = 1
  GROUP BY C.SIGUMGO_HOIKYE_C
UNION ALL 
  SELECT  TO_CHAR(HOIGYE_CODE) HOIGYE_CODE,    
   ...
   WHERE  1=1
     ...
     AND A.GEUMGO_CODE = '42'
     AND A.GUBUN_CODE IN (1, 2, 4)
     AND A.KIJUNIL BETWEEN SUBSTR('20240101',0,4)||'0101' AND '20240702'
     AND A.GUNGU_CODE = 0 
     GROUP BY HOIGYE_CODE
 )
) AA, RPT_HOIKYE_CD BB
