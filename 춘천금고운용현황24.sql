...
FROM (SELECT T1000.MAPP_HOIGYE_CODE
           ...
           , DECODE(110, '42', LPAD(T2000.LINKAC_KWA_C, 3, '0') || LPAD(T2000.LINK_ACSER, 9, '0'), T1000.GONGGEUM_GYEJWA_NO) AS GYEJWA_NO
           , T1000.GONGGEUM_AMT
           , T1000.UY_100_AMT
           , T1000.UY_140_AMT
           , T1000.UY_160_AMT
      ...
      FROM (SELECT T100.MAPP_HOIGYE_CODE
            ...
            FROM (
                ...
                SELECT TO_NUMBER(DECODE(110, 28, T20.ICH_SIGUMGO_HOIKYE_C, T30.SIGUMGO_HOIKYE_C)) AS MAPP_HOIGYE_CODE
...
WHERE T1.GEUMGO_CODE = 110
  ...
  AND T1.GONGGEUM_GYEJWA NOT IN ('04200080900003999')
  AND T1.JANAEK != 0
  and T1.GONGGEUM_GYEJWA = '11000080900000399'
GROUP BY  T1.GEUMGO_CODE
        , T1.GUNGU_CODE
        , T1.GONGGEUM_GYEJWA
...
SELECT T100.MAPP_HOIGYE_CODE
     , T100.GONGGEUM_GYEJWA_NO
     , T100.GONGGEUM_GYEJWA_NM
     , T100.UNYONG_GYEJWA_NO
     , SUM(NVL(T100.GONGGEUM_AMT, 0)) as amt
     , SUM(T200.DCNT) as dcnt
     , TRUNC(SUM(NVL(T100.GONGGEUM_AMT, 0) / T200.DCNT), 8) AS TRNC_GONGGEUM_AMT
     , ROUND(SUM(NVL(T100.GONGGEUM_AMT, 0) / T200.DCNT), 8) AS GONGGEUM_AMT
     , ROUND(SUM(NVL(T100.UY_100_AMT, 0) / T200.DCNT), 8) AS UY_100_AMT
     , ROUND(SUM(NVL(T100.UY_140_AMT, 0) / T200.DCNT), 8) AS UY_140_AMT
     , ROUND(SUM(NVL(T100.UY_160_AMT, 0) / T200.DCNT), 8) AS UY_160_AMT
...
FROM (
    ...
    FROM (SELECT /*+ LEADING(T1 T2 T3) INDEX(T2 RPT_UNYONG_JAN_UK_01) */
          ...
          FROM MAP_JOB_DATE T1
             ...
             , RPT_UNYONG_GYEJWA T3
          WHERE 1 = 1
            and T2.GONGGEUM_GYEJWA = '11000080900000399'
            AND T1.DW_BAS_DDT BETWEEN '20240101' AND '20241231'
            AND T2.KIJUNIL = CASE WHEN T1.DT_G IN (0) THEN T1.DW_BAS_DDT ELSE T1.BF1_BIZ_DT END
            AND T2.GEUMGO_CODE = 110
            AND SUBSTR(T2.GONGGEUM_GYEJWA, 4, 3) = LPAD('0', 3, '0')
...
-- 보통예금
--   11000080900001424, 1100008090000152

select * from rpt_unyong_gyejwa where gonggeum_gyejwa = '11000080900001423'
select * from rpt_unyong_gyejwa where gonggeum_gyejwa = '11000080900001523'
